...
def Preparation_for_optimization_indexed(all_veh_input, interval = 1):
    ...
    for veh_id in veh_id_list:
        ...
        veh_length[veh_id_list.index(veh_id)] = veh_len
...
def replay_results_complete(df_dict, output_root, exp_name, case_index, exp_id):
    ...
    all_veh=df_dict[case_index].copy()# If Lane_ID = 11, change to 1; otherwise, add 1
    all_veh['Lane_ID'] = np.where(
        all_veh['Lane_ID'] == 11,
        1,
        all_veh['Lane_ID'] + 1
...
def optimization_lankershim(
    ...
    M = int(o["M"])
    # rc time is hard coded for the lankershim boulevard scenario
    if case_index in [7,8]:
        rc_time = 0.8
    elif case_index in [11,14,17]:
        rc_time = 0.6
    elif case_index in [12]:
        rc_time = 0.5
    else:
        rc_time = 0.75
    ...
    #flag whether model.x>=90, if yes, b = 0
    model.b = Var(model.V, model.T, domain=Binary)
    ...
    # Auxilliary binary variable to if x is within the no-changing zone
    model.x_nlc = Var(model.V, model.T, domain=Binary)

    # Auxilliary binary variable to if v allows lane-changing
    model.v_nlc = Var(model.V, model.T, domain=Binary)
    ...
    #TODO: minimize lane-changing times
    ...
    def acc_limit_rule(model,v,t):
        if t >= arrival_time[v] and t <= departure_time[v]:
            return (a_low, model.a[v, t], a_high) # first try (-3.5,3.5)
        else:
            return Constraint.Skip
    ...
    '''
    # no_lane_changing zone
    def x_90_rule(model, v, t):
        return model.x[v,t] - 90 <= M * (1 - model.b[v,t])
    model.x_90_con = Constraint(model.V, model.T, rule=x_90_rule)
    
    # 2) Enforce that if b[v,t] = 0 (meaning x[v,t] > 90),
    #    then abs_lc[v,t] must be 0.
    #    We can do this by forcing abs_lc[v,t] <= b[v,t]
    def no_lc_rule(m, v, t):
        return model.abs_lc[v,t] <= model.b[v,t]
    model.no_lc_con = Constraint(model.V, model.T, rule=no_lc_rule)
    '''
    ...
    def car_following_1_rule(model, v, u, t):
        if v != u:
            if t >= arrival_time[v] and t <= departure_time[v] and t >= arrival_time[u] and t <= departure_time[u]:
                return model.x[v,t] - model.x[u,t] >= veh_length[v]*1.2 + rc_time*model.v[u,t] - M * model.z[v, u, t] - M * (1 - model.y[v, u, t])
            else:
                return Constraint.Skip
        else:
            return Constraint.Skip
    ...
    def car_following_2_rule(model, v, u, t):
        if v != u:
            if t >= arrival_time[v] and t <= departure_time[v] and t >= arrival_time[u] and t <= departure_time[u]:
                return model.x[u,t] - model.x[v,t] >= veh_length[u]*1.2 + rc_time*model.v[v,t] - M * model.z[v, u, t] - M * model.y[v, u, t]
            else:
                return Constraint.Skip
        else:
            return Constraint.Skip
    ...
    dfs = [a_df, v_df, x_df, l_df, lc_df, abs_lc_df, l1_df, l2_df, l3_df,l4_df, l5_df]
